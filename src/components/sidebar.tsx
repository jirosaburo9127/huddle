...
export function Sidebar({
  ...
  // 各チャンネルの所属ユーザーID (カテゴリ展開時の在室メンバー一覧に使う)
  ...
  // 各チャンネルの所属ユーザーIDを取得（カテゴリ展開時の在室メンバー表示用）
  useEffect(() => {
    let cancelled = false;
          ...
          >
            ...
            {hitorigotoChannel && (
              <Link
                ...
                className="lg:hidden relative shrink-0 ml-auto mr-1"
              >
                {/* 吹き出し本体 */}
                <div className="relative bg-sidebar border border-border rounded-xl px-3 py-1.5 text-foreground text-sm font-semibold flex items-center gap-1.5 hover:bg-sidebar-hover transition-colors">
                  ...
                  独り言
                  {/* 吹き出しの三角（右向き） */}
                  <div className="absolute -right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 bg-sidebar border-r border-t border-border rotate-45" />
                </div>
...
type ChannelCategoryListProps = {
  ...
  onNavigate: () => void;
  // channel.id → その channel に所属するユーザーID配列
  channelMembersMap: Record<string, string[]>;
  // ワークスペースメンバー（user_id → profile を引くための元ネタ）
...
function ChannelCategoryList({
  ...
  // 各カテゴリ内のチャンネルに所属する全ユーザーID (ユニーク化)
  const memberIdsByCategory = useMemo(() => {
    const result = new Map<string, string[]>();
    for (const [key, list] of grouped.entries()) {
      const set = new Set<string>();
      for (const ch of list) {
        for (const uid of channelMembersMap[ch.id] || []) set.add(uid);
      }
      result.set(key, Array.from(set));
    }
    return result;
  }, [grouped, channelMembersMap]);
