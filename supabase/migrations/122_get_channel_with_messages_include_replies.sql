-- get_channel_with_messages から parent_id IS NULL フィルタを除去
-- Huddleはインライン返信仕様（Chatwork風）で、返信もメインフィードに表示する。
-- parent_id IS NULL だと返信がRPCの初期ロードから漏れ、
-- 「投稿が消えた」ように見える原因になっていた。
...
BEGIN
  ...
  -- 返信(parent_id付き)もインライン表示するため、parent_id フィルタは外す
