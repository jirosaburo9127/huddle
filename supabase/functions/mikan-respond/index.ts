...
# イベント編集ツール (edit_event)
ユーザーが「打合せの時間を3時に変えて」「場所を会議室Bに変更して」「タイトルを〇〇に変えて」と言ったら
\`edit_event\` ツールを呼んでください。
- title_hint: 対象イベントのタイトル（部分一致。省略時はチャンネル内の直近未来イベント）
- new_title: 新しいタイトル（変更する場合のみ）
- new_start_at_iso: 新しい日時 ISO 8601（変更する場合のみ）
- new_location: 新しい場所（変更する場合のみ。空文字で場所をクリア）
- 変更しないフィールドは省略してOK
- ツール呼び出し後は確認メッセージが自動で出るので、テキストでの説明は不要

# チャンネル要約ツール (summarize_channel)
ユーザーが「〇〇チャンネルまとめて」「△△ワークスペースの□□チャンネルを要約して」と言ったら
\`summarize_channel\` ツールを呼んでください。
- channel_name: チャンネル名（部分一致でOK）
- workspace_name: ワークスペース名（任意。指定しないと全WSから検索）
- message_count: 要約するメッセージ数（デフォルト50）
- ツールがチャンネルのメッセージを返すので、それを読んで要約を作成して返す
- 要約は箇条書きで、主要なトピック・決定事項・アクションアイテムを含める
...
const EDIT_EVENT_TOOL = {
  name: "edit_event",
  description:
    "登録済みイベントのタイトル・日時・場所を変更する。" +
    "ユーザーが「打合せの時間を3時に変えて」「場所を会議室Bに変更」「名前を〇〇に変えて」と言ったら呼ぶ。" +
    "変更したいフィールドだけ指定すればよい（指定しないフィールドは変更されない）。",
  input_schema: {
    type: "object",
    properties: {
      title_hint: {
        type: "string",
        description: "対象イベントのタイトル（部分一致）。指定しないとチャンネル内の直近の未来イベント。",
      },
      new_title: {
        type: "string",
      ...
      new_start_at_iso: {
        type: "string",
        description: "新しい開始日時 ISO 8601（変更する場合のみ）。例: '2026-07-01T15:00:00+09:00'",
      },
      new_location: {
        type: "string",
        description: "新しい場所（変更する場合のみ）。空文字で場所をクリア。",
      },
    },
    required: [],
  },
...
const SUMMARIZE_CHANNEL_TOOL = {
  name: "summarize_channel",
  description:
    "指定されたチャンネルの直近メッセージを要約する。" +
    "ワークスペース名やチャンネル名を指定できる。" +
    "「〇〇チャンネルをまとめて」「△△ワークスペースの□□チャンネルの内容を要約して」と言われたら呼ぶ。" +
    "チャンネル名だけ指定した場合は全ワークスペースから検索する。",
  input_schema: {
    type: "object",
    properties: {
      channel_name: {
        type: "string",
        description: "要約対象のチャンネル名（部分一致）",
      },
      workspace_name: {
        type: "string",
        description: "ワークスペース名（任意、部分一致）。指定しないと全ワークスペースから検索。",
      },
      message_count: {
        type: "integer",
        description: "要約対象のメッセージ数（デフォルト50、最大100）",
      },
    },
  ...
  try {
    ...
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        tools: [WEB_SEARCH_TOOL, WEB_FETCH_TOOL, PROPOSE_EVENT_TOOL, SET_EVENT_REMINDER_TOOL, EDIT_EVENT_TOOL, SUMMARIZE_CHANNEL_TOOL],
        system: [
          {
            type: "text",
    ...
    // edit_event ツール
    const editEventBlock = aiJson.content.find(
      (c): c is Extract<ContentBlock, { type: "tool_use" }> =>
        c.type === "tool_use" && c.name === "edit_event"
    );

    if (editEventBlock) {
      const input = editEventBlock.input as {
        title_hint?: string;
        new_title?: string;
        new_start_at_iso?: string;
        new_location?: string;
      };
      ...
      let q = supabase.from("events")
        .select("id, title, start_at, location, message_id")
        .eq("channel_id", msg.channel_id)
      ...
      const target = events[0] as { id: string; title: string; start_at: string; location: string | null; message_id: string | null };

      // 更新内容を組み立て
      const updatedTitle = typeof input.new_title === "string" && input.new_title.trim() ? input.new_title.trim() : target.title;
      const updatedStartAt = typeof input.new_start_at_iso === "string" && input.new_start_at_iso.trim()
        ? new Date(input.new_start_at_iso).toISOString()
        : target.start_at;
      const updatedLocation = typeof input.new_location === "string"
        ? (input.new_location.trim() || null)
        : target.location;
      ...
      if (updateErr) {
        console.error("[mikan] edit_event update failed:", updateErr);
        await supabase.from("messages").insert({
          channel_id: msg.channel_id,
          user_id: MIKAN_USER_ID,
      ...
      if (target.message_id) {
        const locLine = updatedLocation ? `\n📍 ${updatedLocation}` : "";
        await supabase.from("messages")
          .update({
            content: `📅 ${updatedTitle}\n${formatDateTimeJa(updatedStartAt)}${locLine}`,
      ...
      if (input.new_title) changes.push(`タイトル → 「${updatedTitle}」`);
      if (input.new_start_at_iso) changes.push(`日時 → ${formatDateTimeJa(updatedStartAt)}`);
      if (typeof input.new_location === "string") changes.push(`場所 → ${updatedLocation || "（なし）"}`);

      await supabase.from("messages").insert({
        channel_id: msg.channel_id,
        user_id: MIKAN_USER_ID,
        content: `「${target.title}」の予定を更新しました ✏️\n${changes.join("\n")}`,
      });

      return new Response("event edited", { status: 200 });
    }

    // summarize_channel ツール
    const summarizeBlock = aiJson.content.find(
      (c): c is Extract<ContentBlock, { type: "tool_use" }> =>
        c.type === "tool_use" && c.name === "summarize_channel"
    );

    if (summarizeBlock) {
      const input = summarizeBlock.input as { channel_name?: string; workspace_name?: string; message_count?: number };
      ...
      const msgCount = Math.min(typeof input.message_count === "number" ? input.message_count : 50, 100);

      if (!channelName) {
        await supabase.from("messages").insert({
          channel_id: msg.channel_id,
          user_id: MIKAN_USER_ID,
          content: "チャンネル名を教えてください 🤔",
        });
        return new Response("no channel name", { status: 200 });
      }

      // チャンネルを検索（ワークスペース横断 or 指定WS内）
      ...
      if (workspaceName) {
        ...
        if (wsData && wsData.length > 0) {
          channelQuery = channelQuery.eq("workspace_id", wsData[0].id);
        }
      }
      ...
      if (!channelData || channelData.length === 0) {
        await supabase.from("messages").insert({
          channel_id: msg.channel_id,
          user_id: MIKAN_USER_ID,
          content: `「${channelName}」というチャンネルが見つかりませんでした ${workspaceName ? `(${workspaceName}内)` : ""} 🤔`,
        });
        return new Response("channel not found", { status: 200 });
      }

      const targetChannel = channelData[0] as { id: string; name: string; workspace_id: string; workspaces: { name: string } | null };

      // 対象チャンネルのメッセージを取得
      const { data: chMessages } = await supabase
        .from("messages")
      ...
      if (!chMessages || chMessages.length === 0) {
        await supabase.from("messages").insert({
          channel_id: msg.channel_id,
          user_id: MIKAN_USER_ID,
          content: `「${targetChannel.name}」チャンネルにはまだメッセージがありません`,
        });
        return new Response("no messages", { status: 200 });
      }

      // メッセージをコンテキストとしてLLMに要約させる
      const wsName = targetChannel.workspaces?.name || "";
      const messagesText = (chMessages as Array<{ content: string; created_at: string; profiles: { display_name: string } | null }>)
        .reverse()
      ...
      const summaryRes = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: MODEL,
          max_tokens: 2000,
          system: `あなたはチームチャットの要約AIです。以下のチャンネルの会話を要約してください。
要約は以下の形式で:
- 主要なトピック（箇条書き）
- 決定事項があれば（箇条書き）
- 未解決・アクションアイテムがあれば（箇条書き）
柔らかい丁寧語で。`,
          messages: [
            { role: "user", content: `「${targetChannel.name}」チャンネル${wsName ? `（${wsName}）` : ""}の直近${chMessages.length}件の会話:\n\n${messagesText}` },
          ],
        }),
      });

      if (!summaryRes.ok) {
        console.error("[mikan] summary api failed:", summaryRes.status);
        await supabase.from("messages").insert({
          channel_id: msg.channel_id,
          user_id: MIKAN_USER_ID,
          content: "要約の生成に失敗しました 🙏",
        });
        return new Response("summary failed", { status: 200 });
      }

      const summaryJson = await summaryRes.json() as { content: Array<{ type: string; text?: string }> };
      const summaryText = summaryJson.content
        .filter((c) => c.type === "text" && c.text)
        .map((c) => c.text!)
        .join("\n")
        .trim();

      const header = `📝 **${targetChannel.name}**${wsName ? `（${wsName}）` : ""} の要約\n\n`;
      await supabase.from("messages").insert({
        channel_id: msg.channel_id,
        user_id: MIKAN_USER_ID,
      ...
      return new Response("summary posted", { status: 200 });
    }
