...
あなたは Web 検索ツールを使えます。最新情報・固有名詞・統計・公式情報・ユーザーが貼った URL の確認など、
記憶だけで答えるのが危険な内容には **遠慮なく検索してください** (1メッセージで最大 5 回まで)。

## 検索すべき場面
- 「○○について調べて」「○○とは？」と聞かれた時
- ユーザーが URL を貼って「読み込めない？」「中身は？」と尋ねた時
- 「最近の○○」「△△の動向」など現在進行中の話題
...
type ContentBlock =
  ...
  // Anthropic 公式 server tool 系: web_search が API 内部で生成するブロック。
  ...
  try {
    ...
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        // listen モードでは web_search / set_event_reminder は提供しない
        // (見守りモードで勝手に検索/リマインド変更をしないため)
        tools: mode === "mention"
          ? [WEB_SEARCH_TOOL, PROPOSE_EVENT_TOOL, SET_EVENT_REMINDER_TOOL]
          : [PROPOSE_EVENT_TOOL],
        system: [
          {
            type: "text",
