# src/app/(workspace)/[workspace]/albums/components/create-album-modal.tsx (2026-05-18T14:13:27)
...
- [優先度: 今すぐ] `addToAlbumId` 指定時に `channelId` が `channels[0]?.id` のまま使われ、追加先アルバムの実チャンネルと不一致になり得ます。`path = ${channelId}/...` が誤った保存先になるため、既存アルバム追加時はアルバムの `channel_id` を取得して使うか、props で渡してください。

- [優先度: 今すぐ] `accept="image/*,video/*"` なのに `isImageFile(file.name) ? "image" : "video"` としており、PDF 等が混入した場合や拡張子偽装で `video` 扱いになります。`file.type.startsWith("image/")` / `video/` で検証し、対象外は追加前に除外してください。

- [優先度: 今すぐ] `album_items.insert` のエラーを無視しているため、アップロード済みなのにDB未登録の孤児ファイルが発生します。`insert` の `error` を確認し、失敗時は storage の削除、またはユーザー通知と処理中断を行ってください。

- [優先度: 後で] `URL.createObjectURL` のプレビューURLがモーダルを閉じた時にまとめて revoke されません。`useEffect` の cleanup で `previews` を全 revoke するか、ファイル管理を `{ file, previewUrl }` 配列にして削除・unmount 時に確実に解放してください。

- [優先度: 後で] `compressImage` で `canvas.getContext("2d")!` を非null断言しており、取得失敗時にクラッシュします。`const ctx = canvas.getContext("2d"); if (!ctx) { resolve(file); return; }` のようにフォールバックしてください。


## 影響分析 (Claude read-only)

- **何が変わるか**: 既存アルバムに写真を追加するとき、写真が間違った保存先に置かれて表示できなくなる不具合を直します。あわせて、対象外のファイル（PDFなど）が紛れ込んだときの誤判定や、アップロードは成功したのにアルバムに登録されず「迷子の写真」になる事故を防ぎ、画面を閉じたあとのメモリの後始末も改善します。
- **影響範囲**: この画面（アルバム作成・写真追加モーダル）専用の部品で、呼び出しているのは albums のページ1か所だけです（grep で create-album-modal を全 .tsx 検索 → albums/page.tsx のみ）。圧縮処理（compressImage）と画像判定（isImageFile）はこのファイル独自のコピーで、チャット入力やメッセージ表示にある同名の処理は別物・別ファイルなので巻き込みません。ただし保存先チャンネルを正しくするために部品の受け取り情報（props）を増やす場合、呼び出し元の albums/page.tsx 側も合わせて1か所修正が必要になります。
- **LEVEL**: 注意
- **根拠**: 部品自体は1画面専用だが、保存先修正に伴い呼び出し元 albums/page.tsx の修正が連動する可能性があるため。
