# src/components/bottom-tab-bar.tsx (2026-05-13T22:57:06)
...
- [優先度: 今すぐ] 非表示時の「その他メニュー」が `opacity-0 pointer-events-none` だけなので、内部の `Link` / `button` がキーボードフォーカス可能なまま残ります。`className` に `showMore ? "visible ..." : "invisible ..."` を追加するか、各項目に `tabIndex={showMore ? 0 : -1}` を付けて、非表示時にフォーカスできないようにしてください。

- [優先度: 後で] `検索 / DM / ファイル` の `Link` クリック時に `setShowMore(false)` していないため、同一URLへのクリックや遷移失敗時にメニューが開いたままになります。各 `Link` に `onClick={() => setShowMore(false)}` を追加すると挙動が安定します。

- [優先度: 任意] `document.querySelector("[data-more-popover]")` / `data-more-trigger` は同一属性が複数存在した場合に別インスタンスを拾う可能性があります。`useRef<HTMLDivElement | HTMLButtonElement>` を使って、このコンポーネント自身の popover / trigger を参照する形にすると安全です。


## 影響分析 (Claude read-only)

- **何が変わるか**: モバイル画面下部のタブバーで、「その他」メニューを閉じている時の挙動を整える修正です。閉じているはずの裏側メニューが内部的に「触れる」状態のまま残るのを完全に隠し、メニュー内のリンクを押した時に自分自身も閉じるようにします。また、メニュー本体や開閉ボタンを画面全体から探していたのを、自分のコンポーネント内だけを参照する形に変えて誤動作リスクを減らします。
- **影響範囲**: BottomTabBar コンポーネント内で完結する修正です。grep で `BottomTabBar` を検索したところ、参照しているのは `sidebar-loader.tsx` (描画呼び出し1箇所のみ) と、コメントで言及している `main-pane.tsx` / `unread-store.ts` / `sidebar.tsx` だけで、props や export 名は今回変更されません。`data-more-popover` / `data-more-trigger` 属性も grep の結果このファイル内でしか使われておらず、ref 化しても影響範囲は閉じています。
- **LEVEL**: 軽微
- **根拠**: 変更は BottomTabBar 内部の表示制御のみで、外部から参照される export 名や props 構造に変更はない。
