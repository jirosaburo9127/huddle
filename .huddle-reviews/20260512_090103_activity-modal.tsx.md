# src/components/activity-modal.tsx (2026-05-12T09:31:03)
...
- [優先度: 今すぐ] `useEffect(..., [tab, workspaceId])` で `loaded` / 各データ配列をリセットしていないため、`workspaceId` が変わっても前ワークスペースのキャッシュを表示し得ます。`workspaceId` 変更時に `setLoaded(false...)` と `setReactions/mentions/replies([])` を行うか、キャッシュを `workspaceId` 単位で持ってください。

- [優先度: 今すぐ] `router.push(\`/${workspaceSlug}/${channelSlug}?m=${messageId}\`)` が未エンコードです。DB由来の `channelSlug` / `messageId` に `/`, `?`, `#` 等が混じると意図しない遷移になります。`encodeURIComponent(channelSlug)` と `encodeURIComponent(messageId)` を使ってください。

- [優先度: 後で] 既読RPCの失敗を無視したまま `setUnread(...false)` と `huddle:activitySeen` を発火しています。`mark_*_seen` の `error` を確認し、成功時だけ未読ドットを消すようにすると表示とサーバ状態のズレを防げます。

- [優先度: 後で] `a.is_new` 時の背景色が `bg-[#eff6ff]` 固定で、ダークテーマの `text-foreground` と組み合わせるとコントラストが崩れる可能性があります。テーマトークンか `dark:` 指定を使って unread 行の色を定義してください。

- [優先度: 任意] モーダルに `role="dialog"` / `aria-modal="true"`、タブに `role="tablist"` / `role="tab"` / `aria-selected` がありません。キーボード・スクリーンリーダー対応として追加してください。


## 影響分析 (Claude read-only)

- **何が変わるか**: アクティビティ画面で別ワークスペースに切り替えたとき、前のワークスペースの古いデータが残って見えるバグを直します。あわせて、URLに使えない記号がチャンネル名や投稿IDに混ざっても安全にジャンプできるようにします。
- **影響範囲**: ActivityModal は sidebar.tsx からのみ呼ばれており（grep 結果: src/components/sidebar.tsx:1320）、props のシグネチャ（workspaceId / currentChannelSlug / onClose / workspaceSlug）は変わりません。huddle:jumpToMessage イベントは channel-view.tsx が受信していますが、発火タイミングと payload は同じなので影響ありません。huddle:activitySeen も sidebar.tsx 側のドット消去用で payload は不要、変更なし。
- **LEVEL**: 軽微
- **根拠**: 修正範囲は ActivityModal 内部の state リセットとURLエンコードのみで、外部公開APIや発火イベント仕様は変わらない。
