# src/components/sidebar.tsx (2026-05-13T22:15:01)
...
@@ -1119,11 +1125,13 @@ export function Sidebar({
...
+  onNavigate: (isActive: boolean) => void;
   ...
   // ユーザーID → プロフィール解決用
@@ -1924,10 +1932,10 @@ function ChannelCategoryList({
                       }
                     `}
                   >
-                    <Link
...
- [優先度: 今すぐ] `Link` の `onClick` が `isActive` / `pathname === href` のときだけ `setSidebarOpen(false)` するようになっていますが、`onClick` 時点の `pathname` は遷移前の値です。つまり未選択チャンネル/DMをタップしてもモバイルサイドバーが閉じません。`src/components/sidebar.tsx` の hitorigoto、DM、`ChannelCategoryList` は従来通り `onClick={() => setSidebarOpen(false)}` に戻すのが安全です。

- [優先度: 今すぐ] `ChannelCategoryListProps.onNavigate` を `(isActive: boolean) => void` に変更したことで、呼び出し側が「active のときだけ閉じる」という誤った責務を持っています。ナビゲーション操作の完了処理として `onNavigate: () => void` に戻し、子側は `onClick={onNavigate}` に戻してください。

- [優先度: 任意] 変更箇所にタブ混じりのインデントが入っています。`<Link>` 周辺のインデントを既存のスペース基準に揃えてください。


## 影響分析 (Claude read-only)

- **何が変わるか**: モバイルでサイドバーから別のチャンネルやDMをタップしたとき、本来は画面を切り替えてサイドバーが閉じるはずなのに、閉じなくなってしまう不具合の修正案です。ただしレビュアーから「この修正は逆効果」と指摘されています。
- **影響範囲**: `ChannelCategoryList` と `onNavigate` は同ファイル内でしか使われていません (grep で sidebar.tsx 以外にヒットなし)。`Sidebar` 本体は 15 ファイルから参照されていますが、Props は変えていないので外部呼び出しに破壊的影響はありません。影響範囲はサイドバー内のチャンネル/DM/ひとりごとリンクのモバイル挙動のみです。
- **LEVEL**: 軽微
- **根拠**: 変更された型と関数は sidebar.tsx 内部クロージャに閉じており、外部 export されていない。


## Claude Code 修正 (応答)

Codex指摘3件すべて採用。要約:
- hitorigoto/DM/ChannelCategoryListの`onClick`を `setSidebarOpen(false)` に戻した（`pathname`は遷移前の値で未選択タップ時に閉じない不具合だった）
- `onNavigate`を`() => void`に戻し、責務を子側ではなく呼び出し側に集約
- タブ混じりのインデントをスペースに統一
