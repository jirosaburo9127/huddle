# src/components/bottom-tab-bar.tsx (2026-05-15T20:24:05)
...
- [優先度: 今すぐ] 非表示の「その他」メニューが `opacity-0 pointer-events-none` だけなので、内部の `Link` / `button` がキーボード Tab でフォーカス可能です。`aria-hidden` 中のフォーカス可能要素にもなるため、`inert={!showMore}` と `invisible/visible` を併用する、または各項目に `tabIndex={showMore ? 0 : -1}` を入れてください。

- [優先度: 今すぐ] `isHome` が除外リスト方式のため、`/${workspaceSlug}/search` など未列挙の画面でホームがアクティブ表示になります。`pathname === \`/${workspaceSlug}\`` やチャンネル詳細のパターンなど、ホーム扱いするルートを明示する形に直してください。

- [優先度: 後で] 外側タップ判定で `document.querySelector("[data-more-popover]")` / `"[data-more-trigger]"` を使うと、同コンポーネントが複数マウントされた場合に別インスタンスを参照し得ます。`useRef<HTMLDivElement/ButtonElement>` を使って自インスタンス内で判定してください。

- [優先度: 任意] `button` 要素に `type="button"` がありません。将来フォーム配下に配置された場合の意図しない submit を避けるため、ホーム、その他、ブックマーク、メンバーの各 `button` に追加してください。


## 影響分析 (Claude read-only)

- **何が変わるか**: スマホ下部メニューの使い勝手を細かく改善します。隠れている「その他」メニューがキーボード操作で誤って選べてしまう問題を防ぎ、検索など一部画面で「ホーム」が誤って点灯する表示の不具合を直し、メニューが他の場所でも安全に動くようにします。
- **影響範囲**: この部品(BottomTabBar)は sidebar-loader.tsx の1ヶ所からだけ呼ばれており、grep で確認した結果、外部に渡す名前や受け取る項目(workspaceSlug など)の構造は今回まったく変わっていません。変更はすべて部品の内部の表示・操作制御だけなので、サイドバーやチャンネル画面など他の機能には波及しません。未読バッジ用の値も従来どおり同じ仕組みを参照しています。
- **LEVEL**: 影響なし
- **根拠**: BottomTabBar の export 名と props 構造は不変で、変更は同コンポーネント内部の表示制御のみ。
